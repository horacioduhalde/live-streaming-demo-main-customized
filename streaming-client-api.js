    ...
    const talkResponse = await fetch(`${DID_API.url}/talks/streams/${streamId}`, {
      ...
      body: JSON.stringify({
        script: {
          type: 'text',
          subtitles: 'false',
          provider: { type: 'microsoft', voice_id: 'es-AR-ElenaNeural' },
          ssml: false,
...
talkButton.onclick = async () => {
  if (peerConnection?.signalingState === 'stable' || peerConnection?.iceConnectionState === 'connected') {
    ...
    // send the response to the response container
    responseContainer.textContent = responseFromOpenAI;
    //
// test if send to D-ID is active

 // Check if the "Send to DID" checkbox is checked
 const toggleDIDCheckbox = document.getElementById('toggleDID');
 if (toggleDIDCheckbox && toggleDIDCheckbox.checked) {
   // Only call handleDIDStreaming if the checkbox is checked
  handleDIDStreaming(responseFromOpenAI);
 } else {
   console.log("DID streaming is toggled off. Not sending to DID.");
 }
